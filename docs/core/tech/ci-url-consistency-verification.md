...
- **関連ドキュメント**: `docs/core/tech/incident-report-indexing.md`（インシデント報告書）、`morihara-tech/build-tools#58`（CloudFront Function側の恒久対応）
...
2. **③をsandbox-deploy.yml（PRごとのサンドボックス）で検証**: サンドボックスは `s3://bucket/pr-{番号}/` というパスプレフィックス配信であり、CloudFront Functionのリダイレクトロジック（`/ja` → `/ja/` 等、ドメインルート基準の想定が濃厚）が同一に発火する保証がない。Function実装は別リポジトリで不可視のため裏付けが取れず、誤った安心感を与えるリスクが高いため③の主検証としては不採用。
...
scripts/
  ...
  url-policy.mjs                       新規。CloudFront Function側の「意図されたリダイレクトルール」を
                                        ...
                                        （build-tools#58 の実装と対応させる。コメントで参照元を明記）
...
2. `node scripts/verify-url-consistency.mjs` を実行
   ...
   - `scripts/url-policy.mjs` から「CloudFront Functionが301する既知のパターン」（例: `/ja` 単体、`/en` 単体）を取得
...
4. **異常系**: テストコード内でcanonical URLの一部を意図的にモックのリダイレクト対象パス（例: `/ja`）に書き換えたコピーを用意し、同じ判定ロジックを実行して非ゼロ終了（失敗）になることを`assert`
...
| ファイル | 役割 |
|---|---|
| `scripts/url-policy.mjs` | CloudFront Functionの意図されたリダイレクトルールを本リポジトリ側で保持する唯一の定義。`build-tools#58`の実装と対応させ、コメントで参照元URLを明記。Layer A（既知パターンとの一致チェック）とLayer B（モックサーバーの挙動定義）の両方から参照する共通ソース |
...
| フェーズ | 実装範囲 | 備考 |
|---|---|---|
| フェーズ1 | `scripts/url-policy.mjs` 新規作成（既知のリダイレクトパターンを定義。当面は `/ja` `/en` の2パターンのみ。`build-tools#58`の実装確認後に確定） | 別リポジトリ側の実装確認が前提。不明な場合はCOO経由で `build-tools` 側の管理者に既知パターンの一覧を確認する |
...
- `url-policy.mjs` の内容が `build-tools#58` の実装と乖離しないよう、運用ルール（`build-tools`側でリダイレクトルールを変更する際は本リポジトリの`url-policy.mjs`も同時更新するクロスリポジトリ手順）を別途文書化する必要がある。理想的には`build-tools`側がリダイレクトルールをJSON等でエクスポートし、本リポジトリがそれをfetchして単一の真実源にする長期改善が望ましいが、本設計のスコープ外とし`build-tools#58`側への提案事項とする。
